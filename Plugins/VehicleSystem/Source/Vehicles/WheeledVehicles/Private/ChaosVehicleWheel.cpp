...
UChaosVehicleWheel::UChaosVehicleWheel(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer) {
...
float UChaosVehicleWheel::GetSteerAngle() const {
	...
	return VehicleComponent->PVehicleOutput->Wheels[WheelIndex].SteeringAngle;
}

float UChaosVehicleWheel::GetRotationAngle() const {
	check(VehicleComponent && VehicleComponent->PVehicleOutput);
	float RotationAngle = -1.0f * FMath::RadiansToDegrees(VehicleComponent->PVehicleOutput->Wheels[WheelIndex].AngularPosition);
...
float UChaosVehicleWheel::GetRotationAngularVelocity() const {
	check(VehicleComponent && VehicleComponent->PVehicleOutput);
	float RotationAngularVelocity = -1.0f * FMath::RadiansToDegrees(VehicleComponent->PVehicleOutput->Wheels[WheelIndex].AngularVelocity);
...
float UChaosVehicleWheel::GetWheelRadius() const {
	check(VehicleComponent && VehicleComponent->PVehicleOutput);
	return VehicleComponent->PVehicleOutput->Wheels[WheelIndex].WheelRadius;
}

float UChaosVehicleWheel::GetWheelAngularVelocity() const {
	check(VehicleComponent && VehicleComponent->PVehicleOutput);
	return VehicleComponent->PVehicleOutput->Wheels[WheelIndex].AngularVelocity;
}
...
bool UChaosVehicleWheel::IsInAir() const {
	check(VehicleComponent && VehicleComponent->PVehicleOutput);
	return !VehicleComponent->PVehicleOutput->Wheels[WheelIndex].InContact;
}
