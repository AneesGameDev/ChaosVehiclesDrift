...
#include "ChaosVehicleElevator.generated.h"

class UPhysicalMaterial;
class FChaosVehicleManager;
class UChaosHoveringVehicleMovementComponent;

	UCLASS(BlueprintType, Blueprintable)
		class HOVERINGVEHICLES_API UChaosVehicleElevator : public UChaosVehicleMover
	{
		...
		UPROPERTY(transient)
		TObjectPtr<class UChaosHoveringVehicleMovementComponent> VehicleComponent;
		 ...
		 */
		virtual void Init(class UChaosHoveringVehicleMovementComponent* InVehicleSim, int32 InWheelIndex);
		 ...
		 */
		struct FChaosElevatorSetup& GetElevatorSetup();
	...
	private:
		...
		void FillWheelSetup()
		{
			...
			PElevatorConfig.Offset = this->Offset;
			//PWheelConfig.WheelMass = this->WheelMass;
			PElevatorConfig.WheelRadius = this->WheelRadius;
			PElevatorConfig.AxleType = static_cast<Chaos::FSimpleElevatorConfig::EAxleType>(this->AxleType);
			PElevatorConfig.LateralSlipGraph.Empty();
		...
		Chaos::FSimpleElevatorConfig PElevatorConfig;
		Chaos::FSimpleSuspensionConfig PSuspensionConfig;

	public:

		const Chaos::FSimpleElevatorConfig& GetPhysicsElevatorConfig()
		{
			FillWheelSetup();
			return PElevatorConfig;
		}
