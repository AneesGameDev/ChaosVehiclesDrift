...
namespace Chaos
{
	void FAerofoil::Simulate(const FControlInputs& ControlInput) {
			switch (Setup().Type)
			{
			case Chaos::EAerofoilType::Rudder:
				SetControlSurface(-ControlInput.Yaw);
				break;

			case Chaos::EAerofoilType::Elevator:
				SetControlSurface(ControlInput.Pitch);
				break;

			case Chaos::EAerofoilType::Wing:
				if (Setup().Offset.Y < 0.0f)
				{
					SetControlSurface(ControlInput.Roll);
				}
				else
				{
					SetControlSurface(-ControlInput.Roll);
				...
				break;
			case EAerofoilType::Fixed: break;
			default: ;
			}
	}
