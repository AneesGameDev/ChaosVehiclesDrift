...
struct FSuspensionUtility
{
	...
	VEHICLEPARTS_API static bool ComputeSprungMasses(const TArray<FVector>& MassSpringPositions, const float TotalMass, TArray<float>& OutSprungMasses);
	...
	/** Same as above, but allows the caller to specify spring locations
		...
		of mass. */
	VEHICLEPARTS_API static bool ComputeSprungMasses(const TArray<FVector>& LocalSpringPositions, const FVector& LocalCenterOfMass, const float TotalMass, TArray<float>& OutSprungMasses);

	/** Natural frequency of spring in radians/sec - divide by 2Pi to get result in Hz */
	VEHICLEPARTS_API static float ComputeNaturalFrequency(float SpringRate, float SprungMass)
	{
		return FMath::Sqrt(SpringRate / SprungMass);
	}

	/** Compute spring damping value that will achieve critical damping */
	VEHICLEPARTS_API static float ComputeCriticalDamping(float SpringRate, float SprungMass)
	{
	...
	VEHICLEPARTS_API static float ComputeDamping(float SpringRate, float SprungMass, float DampingRatio)
	{
